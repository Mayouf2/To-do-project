...
export default function ToDoList({token}) {
  ...
  const addTask =async (id)=>{
    if(!task){
      ...
      const result = await axios.post(`http://localhost:5000/task/${id}`, {
        newName:name,
        newTask:task,
        newCheck:check,
...
const deletedTask = await axios.delete(`http://localhost:5000/task/${id}`,{
  headers:{authorization: "Bearer " + token},
});
...
const updateCheck = async (id,check)=>{
  const result = await axios.put(`http://localhost:5000/task/${id}`,{check:!check}, {
    headers:{authorization: "Bearer " + token},
    },    
    );
